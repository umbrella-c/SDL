...
void SdlWindow::DescriptorEvent(int iod, unsigned int events)
{
...
void SdlWindow::CreateWindowBuffer(enum PixelFormat format)
{
...
int SDL_VALI_CreateWindow(_THIS, SDL_Window * window)
{
    ...
    SDL_GetWindowSize(window, &x, &h);
    auto window = APP.CreateWindow<SdlWindow>(Asgaard::Rectangle(0, 0, w, h));
    if (window == nullptr) {
        return -1;
    }
    
    SDL_SetWindowData(window, VALI_WINDOW_DATA, window.get());
    return 0;
}
...
void SDL_VALI_DestroyWindow(_THIS, SDL_Window * window)
{
    ...
    OM.DestroyObject<SdlWindow>(window->Id());
}
